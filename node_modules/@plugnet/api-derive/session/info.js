...
function createDerived0to94(_ref) {
  ...
  return {
    ...
    eraProgress: currentEra.mul(sessionLength).add(sessionProgress),
    isEpoch: false,
    lastEraLengthChange,
    lastLengthChange,
    sessionLength,
...
function createDerived(_ref2) {
  let [[epochDuration, sessionsPerEra], [currentSlot, epochIndex, epochStartSlot, currentIndex, currentEra]] = _ref2;
  const sessionProgress = currentSlot.sub(epochStartSlot);
  const eraProgress = epochIndex.mod(sessionsPerEra).mul(epochDuration).add(sessionProgress); // FIXME This alwasy assumes Babe, as per the substrate defaults - at least for
...
function info(api) {
  return () => {
    ...
    (0, _rxjs.of)([api.consts.babe.epochDuration, api.consts.staking.sessionsPerEra]), api.queryMulti([api.query.babe.currentSlot, api.query.babe.epochIndex, api.query.babe.epochStartSlot, api.query.session.currentIndex, api.query.staking.currentEra])]).pipe((0, _operators.map)(createDerived), (0, _drr.drr)());
  };
}