...
var _apiDerive = _interopRequireDefault(require("@plugnet/api-derive"));

var _rpcCore = _interopRequireDefault(require("@plugnet/rpc-core"));

var _rpcProvider = require("@plugnet/rpc-provider");

var _types = require("@plugnet/types");

var _Linkage = require("@plugnet/types/codec/Linkage");
...
class Decorate extends _Events.default {
  ...
  constructor(options, type, decorateMethod) {
    ...
    this._type = type;
    this._rpcCore = new _rpcCore.default(thisProvider);
  ...
  decorateRpc(rpc, decorateMethod) {
    return ['author', 'chain', 'state', 'system'].reduce((out, _sectionName) => {
      const sectionName = _sectionName; // out and section here are horrors to get right from a typing perspective :()

      out[sectionName] = Object.entries(rpc[sectionName]).reduce((section, _ref) => {
  ...
  decorateExtrinsics(extrinsics, decorateMethod) {
    ...
    return Object.entries(extrinsics).reduce((out, _ref2) => {
      let [name, section] = _ref2;
      out[name] = Object.entries(section).reduce((out, _ref3) => {
        let [name, method] = _ref3;
  ...
  decorateStorage(storage, decorateMethod) {
    return Object.entries(storage).reduce((out, _ref4) => {
  ...
  decorateStorageEntry(creator, decorateMethod) {
    ...
    const decorated = creator.headKey ? this.decorateStorageEntryLinked(creator, decorateMethod) : decorateMethod(function () {
      return _this._rpcCore.state // Unfortunately for one-shot calls we also use .subscribeStorage here
    ...
    decorated.key = (arg1, arg2) => (0, _util.u8aToHex)((0, _util.compactStripLength)(creator(creator.meta.type.isDoubleMap ? [arg1, arg2] : arg1))[1]); // When using double map storage function, user need to pass double map key as an array
  ...
  decorateStorageEntryLinked(creator, decorateMethod) {
    ...
    const getNext = key => this._rpcCore.state.subscribeStorage([[creator, key]]).pipe((0, _operators.switchMap)((_ref7) => {
      let [data] = _ref7;
    ...
    return decorateMethod(function () {
      ...
      return args.length ? _this2._rpcCore.state.subscribeStorage([[creator, ...args]]).pipe((0, _operators.map)((_ref8) => {
        let [data] = _ref8;
        return data;
      })) : _this2._rpcCore.state.subscribeStorage([creator.headKey]).pipe((0, _operators.switchMap)((_ref9) => {
        let [key] = _ref9;
