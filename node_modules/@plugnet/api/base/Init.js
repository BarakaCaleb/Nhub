...
var _fromMetadata = _interopRequireDefault(require("@plugnet/api-metadata/consts/fromMetadata"));

var _fromMetadata2 = _interopRequireDefault(require("@plugnet/api-metadata/extrinsics/fromMetadata"));

var _fromMetadata3 = _interopRequireDefault(require("@plugnet/api-metadata/storage/fromMetadata"));

var _types = require("@plugnet/types");

var _constants = require("@plugnet/types/primitive/Extrinsic/constants");

var _util = require("@plugnet/util");

var _utilCrypto = require("@plugnet/util-crypto");
...
const KEEPALIVE_INTERVAL = 15000; // these are override types for polkadot chains
...
class Init extends _Decorate.default {
  constructor(options, type, decorateMethod) {
    ...
    (0, _util.assert)(this._rpcCore.provider.hasSubscriptions, 'Api can only be used with a provider supporting subscriptions'); // We only register the types (global) if this is not a cloned instance.
  ...
  async metaFromChain(optMetadata) {
    ...
    const [genesisHash, runtimeVersion, chain] = await Promise.all([this._rpcCore.chain.getBlockHash(0).toPromise(), this._rpcCore.state.getRuntimeVersion().toPromise(), this._rpcCore.system.chain().toPromise()]);
    const specName = runtimeVersion.specName.toString(); // based on the node spec & chain, inject specific type overrides

    this.registerTypes(_objectSpread({}, TYPES_SPEC[specName] || {}, {}, typesSpec[specName] || {}, {}, typesChain[chain.toString()] || {})); // retrieve metadata, either from chain  or as pass-in via options
    ...
    this._runtimeVersion = runtimeVersion; // get unique types & validate
  ...
  async initFromMeta(metadata) {
    ...
    if (!this._options.source) {
      ...
      _types.GenericCall.injectMethods(extrinsics); // detect the extrinsic version in-use based on the last block
      ...
      this._extrinsicType = firstTx ? firstTx.type : _constants.LATEST_VERSION;
    }
