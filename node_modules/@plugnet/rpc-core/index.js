...
var _jsonrpc = _interopRequireDefault(require("@plugnet/jsonrpc"));

var _types = require("@plugnet/types");

var _codec = require("@plugnet/types/codec");
...
class Rpc {
  ...
  createInterface(section) {
    const {
      methods
    } = _jsonrpc.default[section];
    return Object.keys(methods).reduce((exposed, methodName) => {
      const def = methods[methodName]; // FIXME Remove any here
  ...
  createMethodSend(method) {
    ...
    const call = function call() {
      ...
      return (0, _rxjs.of)(1).pipe((0, _operators.map)(() => _this.formatInputs(method, values)), (0, _operators.switchMap)(params => (0, _rxjs.combineLatest)([(0, _rxjs.of)(params), (0, _rxjs.from)(_this.provider.send(rpcName, params.map(param => param.toJSON())))])), (0, _operators.map)((_ref3) => {
        let [params, result] = _ref3;
    ...
    const call = function call() {
      ...
      return new _rxjs.Observable(observer => {
        ...
        try {
          ...
          subscriptionPromise = _this2.provider.subscribe(subType, subName, paramsJson, update).catch(errorHandler);
        } catch (error) {
          errorHandler(error);
        } // Teardown logic
  ...
  formatOutput(method, params, result) {
    const base = (0, _codec.createTypeUnsafe)(method.type, [result]);
  ...
  formatStorageData(key, base, isNull) {
    ...
    if (meta.modifier.isOptional) {
      return new _types.Option((0, _types.createClass)(type), isNull ? null : (0, _codec.createTypeUnsafe)(type, [base], true));
    }

    return (0, _codec.createTypeUnsafe)(type, [isNull ? meta.fallback : base], true);
  }
  ...
  formatStorageSet(key, base, witCache) {
    ...
    const meta = key.meta || EMPTY_META; // if we don't find the value, this is out fallback
    //   - in the case of an array of values, fill the hole from the cache
    //   - if a single result value, don't fill - it is not an update hole
    //   - fallback to an empty option in all cases

    const emptyVal = (witCache ? this._storageCache.get(hexKey) : null) || new _types.Option((0, _types.ClassOf)('StorageData'), null); // see if we have a result value for this specific key, fallback to the cache value
    // when the value in the set is not available, or is null/empty.

    const [, value] = base.changes.find((_ref5) => {
      let [key, value] = _ref5;
      return value.isSome && key.toHex() === hexKey;
    }) || [null, emptyVal]; // store the retrieved result - the only issue with this cache is that there is no
    ...
    if (meta.modifier.isOptional) {
      return new _types.Option((0, _types.createClass)(type), value.isNone ? null : (0, _codec.createTypeUnsafe)(type, [value.unwrap()], true));
    }

    return (0, _codec.createTypeUnsafe)(type, [value.unwrapOr(meta.fallback)], true);
  }
