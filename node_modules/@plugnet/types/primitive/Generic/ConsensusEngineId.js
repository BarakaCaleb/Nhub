...
class ConsensusEngineId extends _U.default {
  ...
  getAuraAuthor(bytes, sessionValidators) {
    return sessionValidators[(0, _create.createType)('RawAuraPreDigest', bytes.toU8a(true)).slotNumber.modn(sessionValidators.length)];
  }

  getBabeAuthor(bytes, sessionValidators) {
    const digest = (0, _create.createType)('RawBabePreDigest', bytes.toU8a(true));
    return sessionValidators[(digest.isPrimary ? digest.asPrimary : digest.asSecondary).authorityIndex.toNumber()];
  }
  /**
