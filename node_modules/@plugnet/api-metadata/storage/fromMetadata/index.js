...
function fromMetadata(metadata) {
  return metadata.asLatest.modules.reduce((result, moduleMetadata) => {
    ...
    const prefix = unwrapped.prefix.toString(); // For access, we change the index names, i.e. Balances.FreeBalance -> balances.freeBalance

    result[section] = unwrapped.items.reduce((newModule, meta) => {
      const method = meta.name.toString();
      newModule[(0, _util.stringLowerFirst)(method)] = (0, _createFunction.default)({
        meta,
        method,
        prefix,
        section
    ...
    return result;
  }, _objectSpread({}, _storage.storage));
}